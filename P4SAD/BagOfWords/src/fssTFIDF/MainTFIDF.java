...
public class MainTFIDF {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length > 3) { //si hay más de 3 parámetros
			...
			Filter filter = Tfidf.getTfidf().getTFIDFFilter(data);
			...
			s.setFile(new File(varSubstring + "/trainBOW_FSS_TFIDF.arff"));
			s.writeBatch();
			
			s.setInstances(dataDevFiltered);
			s.setFile(new File(varSubstring + "/devBOW_FSS_TFIDF.arff"));
			s.writeBatch();
			
			s.setInstances(testFiltered);
			s.setFile(new File(varSubstring + "/test_blindBOW_FSS_TFIDF.arff"));
			s.writeBatch();
		}
	}
